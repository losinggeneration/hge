...
var (
	...
	sky, sun, moon, glow, seaglow, star HGE.Sprite
	sea                                 HGE.DistortionMesh
	colWhite                            HGE.ColorRGB
)
...
var (
	...
	colSkyTop, colSkyBtm, colSeaTop, colSeaBtm HGE.ColorRGB

	colSun, colSunGlow, colMoon, colMoonGlow, colSeaGlow HGE.ColorRGB
...
func render() int {
	...
	hge.Gfx_BeginScene()
	RenderSimulation()
	fnt.Printf(7, 7, HGE.TEXT_LEFT, "Keys 1-9 to adjust simulation speed, 0 - real time\nFPS: %d", hge.Timer_GetFPS())
	fnt.Printf(SCREEN_WIDTH-50, 7, HGE.TEXT_LEFT, "%02d:%02d:%02d", hrs, mins, secs)
	hge.Gfx_EndScene()
...
func InitSimulation() bool {
	...
	// Create sprites
	sky = HGE.NewSprite(0, 0, 0, SCREEN_WIDTH, SKY_HEIGHT)
	sea = HGE.NewDistortionMesh(SEA_SUBDIVISION, SEA_SUBDIVISION)
	sea.SetTextureRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-SKY_HEIGHT)

	sun = HGE.NewSprite(texObjects, 81, 0, 114, 114)
	sun.SetHotSpot(57, 57)
	moon = HGE.NewSprite(texObjects, 0, 0, 81, 81)
	moon.SetHotSpot(40, 40)
	star = HGE.NewSprite(texObjects, 72, 81, 9, 9)
	star.SetHotSpot(5, 5)

	glow = HGE.NewSprite(texObjects, 128, 128, 128, 128)
	glow.SetHotSpot(64, 64)
	glow.SetBlendMode(HGE.BLEND_COLORADD | HGE.BLEND_ALPHABLEND | HGE.BLEND_NOZWRITE)
	seaglow = HGE.NewSprite(texObjects, 128, 224, 128, 32)
...
func UpdateSimulation() {
	...
	for i := 1; i < SEA_SUBDIVISION-1; i++ {
		...
		for j := 0; j < SEA_SUBDIVISION; j++ {
			...
			sea.SetDisplacement(j, i, 0.0, dy, HGE.DISP_NODE)
		}
	}
