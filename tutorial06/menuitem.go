...
type GUIMenuItem struct {
	...
	delay                                                             float64
	title                                                             string
	scolor, dcolor, scolor2, dcolor2, sshadow, dshadow, color, shadow HGE.ColorRGB
	soffset, doffset, offset                                          float64
	timer, timer2                                                     float64
}

func NewGUIMenuItem(id int, font *HGE.Font, snd HGE.Effect, x, y, delay float64, title string) *HGE.GUIObject {
	mi := new(GUIMenuItem)
	...
	mi.GUIObject.Render = func() {
		...
		mi.font.Render(mi.GUIObject.Rect.X1+mi.offset+3, mi.GUIObject.Rect.Y1+3, HGE.TEXT_LEFT, mi.title)
		mi.font.SetColor(mi.color.GetHWColor())
		mi.font.Render(mi.GUIObject.Rect.X1-mi.offset, mi.GUIObject.Rect.Y1-mi.offset, HGE.TEXT_LEFT, mi.title)
	}
